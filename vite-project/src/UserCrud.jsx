...
const UserCrud = () => {
    ...
    const deleteUsers=(id) => {
        ...
        if(window.confirm("Sei sicuro di voler cancellare questo utente?")){
            setUsers(newUsers)
            // poi teoricamnete dovremmo aggiornare la lista così che clementina se ne possa andare definitivamenbte
        }
    }
    ...
    <div className="col-sm-10">
        ...
        <div className="row">
        <div className="col-8 p-2 d-flex justify-content-start bg-secondary text-white">Utente</div>
        <div className="col-4 p-2 d-flex justify-content-end bg-secondary text-white">Azioni</div>
        </div>
        <div>
            
            {
                users.map((u) => {
                    ...
                    <div className="col-4 p-2 d-flex justify-content-end gap-2">
                        ...
                        <button className="btn btn-danger" onClick={()=>{deleteUsers(u.id)}}>Delate</button>
                    </div>
                    </div>
                    );
                }
